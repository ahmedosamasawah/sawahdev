...
for (const route of manifest.routes) {
    ...
    if (!segments.length) throw new Error(`invalid path in route-manifest.json: "${route.path}"`)

    segments[0] = ':locale'

    if (route.type === 'blog-post' && segments.length > 2) segments[segments.length - 1] = ':slug'

    const pattern = '/' + segments.join('/')
    pattern_set.add(pattern)
}

const routes = Array.from(pattern_set).map(pattern => [pattern, {}] as [string, object])
