<script>
	...
	const post = $derived(blog_posts.find((p) => p.locale === locale && p.slug === slug));
...
{#if post}
	<div class="mx-auto w-full max-w-7xl px-6 lg:flex lg:justify-between lg:gap-8 lg:px-24">
		...
		<main class="pt-12 lg:w-[52%] lg:py-24">
			...
			<article class="blog-article prose prose-sm dark:prose-invert max-w-none" lang={article_lang}>
				{@html post.html}
			</article>
